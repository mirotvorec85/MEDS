...
import meds.Damage;
import meds.Inventory;
import meds.Item;
import meds.Player;
import meds.ServerOpcodes;
import meds.ServerPacket;
import meds.Unit;
import meds.UnitTypes;
import meds.database.DBStorage;
import meds.enums.ItemBonusParameters;
import meds.enums.ItemClasses;
import meds.enums.ItemSubClassWeapon;
import meds.enums.Parameters;
import meds.util.Random;

public class Spell
{
     private meds.database.entity.Spell entry;
     private Unit caster;
     private Unit target;
     private int level;
     private Item item;

     public Spell(meds.database.entity.Spell entry, Unit caster, int level, Unit target, Item item)
     {
         this.entry = entry;
         this.caster = caster;
         this.level = level;
     ...
     public Spell(meds.database.entity.Spell entry, Unit caster, int level, Unit target)
     {
         this(entry, caster, level, target, null);
     }

     public Spell(meds.database.entity.Spell entry, Unit caster, int level)
     {
     ...
     private void handleSpellBattleMagic()
     {
         ...
         damage.setInitialDamage(Random.nextInt(minDamage, maxDamage));
         ...
         if (this.entry.getId() == 3)
         {
             damage.setInitialDamage((int)(damage.getInitialDamage() * (Random.nextDouble() * 0.6 - 0.3 + 1)));
         }
         ...
         this.caster.dealDamage(this.target, damage);
     }
